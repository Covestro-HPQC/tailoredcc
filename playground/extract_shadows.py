...
# print(vqe.vqe_energy())
# np.random.seed(42)
# vqe.params += np.random.randn(*vqe.params.shape) * 1e-2
# opt = cov.LBFGSB(atol=1e-14, gtol=None, ftol=None)
# vqe.vqe_optimize(opt=opt, maxiter=1000)
...
for shadow_size in sizes:
    ...
    for ii in range(nsamp):
        ...
        start = time.time()
        shadow_overlaps = np.array(shadow_method.overlap(None, ansatz=ansatz, states=states))
        ...
        np.savez(
            f"shadow_records/shadow_h4_{shadow_size}_{ii}",
