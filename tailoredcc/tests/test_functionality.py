...
def test_from_fqe_wfn():
    ...
    from tailoredcc.workflow import ec_cc_from_fqe, tccsd_from_fqe
    ...
    # rotate by global phase
    wfn_complex = pyscf_to_fqe_wf(mc.ci * np.exp(1j * 0.42), norbs=ncas, nelec=nelec)
    ret_fqe_complex = tccsd_from_fqe(scfres, wfn_complex)
    np.testing.assert_allclose(ret_fqe_complex.e_tot, ret_ci.e_tot, atol=1e-9, rtol=0)
