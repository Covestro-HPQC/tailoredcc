...
AnyCC = Union[cc.ccsd.CCSD, TCC]


def tccsd_from_ci(mc: mcscf.casci.CASCI, backend="pyscf", **kwargs) -> AnyCC:
    """High-level function to run TCCSD based on a PySCF CASCI calculation.

    Parameters
    ----------
    mc : mcscf.casci.CASCI
        PySCF CASCI result object
    backend : str, optional
        Backend to run the TCCSD calculation, by default "pyscf"
...
def tccsd_from_fqe(
    ...
    **kwargs,
    ...
    """High-level function to run TCCSD based on an FQE wavefunction.
...
def tccsd_pyscf(
    ...
    **kwargs,
    ...
    """Low-level entrypoint to run TCCSD with PySCF.
    ...
    scfres : scf.hf.SCF
        PySCF SCF result
    c_ia : npt.NDArray
        C1 coefficients in active spin-orbitals
    c_ijab : npt.NDArray
        C2 coefficients in active spin-orbitals
    occslice : slice
        Slice to the active occupied orbital space in the full MO space
    virtslice : slice
        Slice to the active virtual orbital space in the full MO space
...
def compute_integrals_custom(
    pyscf_molecule: gto.Mole, pyscf_scf: scf.hf.SCF, mo_coeff: npt.NDArray
) -> Tuple[npt.NDArray, npt.NDArray]:
    """Compute 1- and 2-electron integrals for given MO coefficients

    Parameters
    ----------
    pyscf_molecule : gto.Mole
        PySCF molecule instance
    pyscf_scf : scf.hf.SCF
        PySCF HF object
    mo_coeff : npt.NDArray
        MO coefficients
...
def tccsd_opt_einsum(
    ...
    **kwargs,
    ...
    """Low-level entrypoint to TCCSD implementation with opt_einsum.
    ...
    -------
    TCC
        TCCSD calculation result container
    """
    warnings.warn(
...
def ec_cc_from_ci(mc: mcscf.casci.CASCI, **kwargs):
    """High-level function to run ec-CC based on a PySCF CASCI calculation.
    ...
    return ec_cc(mc._scf, c1, c2, c3, c4, occslice, virtslice, **kwargs)
...
def ec_cc_from_fqe(
    ...
    **kwargs,
    ...
    """High-level function to run ec-CC based on an FQE wavefunction.
...
def ec_cc(
    ...
    t1_guess: Union[npt.NDArray, None] = None,
    t2_guess: Union[npt.NDArray, None] = None,
